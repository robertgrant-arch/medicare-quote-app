...
function estimateDrugCost(plan: MedicarePlan, drugs: RxDrug[], subsidy: "full" | "partial" | "none"): number {
  if (drugs.length === 0) return 0;
  const ICL = 5030;
  const CATASTROPHIC = 8000;
  let totalAnnual = 0;
  for (const drug of drugs) {
    const tierKey = drug.isGeneric ? "tier1" : "tier2";
    const copayStr = plan.rxDrugs[tierKey];
    const match = copayStr.match(/\$(\d+)/);
    const coinsMatch = copayStr.match(/(\d+)%/);
    let monthlyCost = 0;
    if (match) monthlyCost = parseInt(match[1], 10);
    else if (coinsMatch) monthlyCost = (parseInt(coinsMatch[1], 10) / 100) * (drug.isGeneric ? 30 : 150);
    else monthlyCost = drug.isGeneric ? 5 : 47;
    totalAnnual += monthlyCost * 12;
  }
  if (subsidy === "full") return Math.round(totalAnnual * 0.05);
  if (subsidy === "partial") return Math.round(totalAnnual * 0.25);
  if (totalAnnual > ICL && !plan.rxDrugs.gap) {
    const initial = ICL;
    const gapSpend = Math.min(totalAnnual - ICL, CATASTROPHIC - ICL) * 0.25;
    const catastrophic = totalAnnual > CATASTROPHIC ? (totalAnnual - CATASTROPHIC) * 0.05 : 0;
    return Math.round(initial + gapSpend + catastrophic);
  }
...
export default function PlanCard({
  ...
  const drugCost = rxDrugs.length > 0 ? estimateDrugCost(plan, rxDrugs, subsidyLevel) : null;
  ...
  return (
    ...
    >
      ...
      {/* Drug Cost Estimate */}
      {drugCost !== null && (
        <div className="mx-4 mt-2 p-2.5 rounded-lg flex items-center gap-2" style={{ backgroundColor: "#FFF7ED", border: "1px solid #FED7AA" }}>
          <Pill size={14} className="text-orange-500 shrink-0" />
          <div className="flex-1">
            <p className="text-xs font-bold" style={{ color: "#9A3412" }}>Est. Drug Cost: ${drugCost.toLocaleString()}/yr</p>
            <p className="text-[9px] text-orange-400">Based on {rxDrugs.length} medication{rxDrugs.length > 1 ? "s" : ""}{subsidyLevel !== "none" ? ` · ${subsidyLevel} subsidy` : ""}</p>
          </div>
        </div>
      )}
